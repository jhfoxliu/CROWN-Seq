...
if __name__ == "__main__":
	...
	for seq in SeqIO.parse(options.fasta,"fasta"):
		dictLength[seq.id.split("|")[0]] = len(seq.seq)
			...
			if type == "transcript":
				...
				if options.set_all_coding == False:
					try:
						...
						raise Warning("tag: gene_type not in GTF file, please check if nothing wrong. If no biotype available, you can use --all-coding or --all-unknown to force a biotype to use")
